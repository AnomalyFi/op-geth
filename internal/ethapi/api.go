...
// CallAtomicArgs represents the arguments for a call.
type CallAtomicArgs struct {
...
// CallAtomic will simulate a transaction at the top of a given block
// number with the state of another (or the same) block. This can be used to
// simulate future blocks with the current state, or it can be used to simulate
// a past block.
// The sender is responsible for signing the transaction and using the correct
// nonce and ensuring validity
// TODO this is what I want to use to simulate the atomic txs
func (s *BundleAPI) CallAtomic(ctx context.Context, args CallAtomicArgs) (map[string]interface{}, error) {
	...
	if args.BaseFee != nil {
		...
		baseFee = eip1559.CalcBaseFee(s.b.ChainConfig(), parent, timestamp)
	}
	header := &types.Header{
	...
	for i, tx := range txs {
		...
		logResults := []map[string]interface{}{}
		for j, log := range receipt.Logs {

			//TODO unsure if I need to convert this to a string or not
			res, err := log.MarshalJSON()

			if err != nil {
				return nil, fmt.Errorf("err: %w; txhash %s", err, tx.Hash())
			}

			logResultRaw := map[string]interface{}{}

			logResultRaw[fmt.Sprint(j)] = res

			logResults = append(logResults, logResultRaw)
		}
		jsonResult["logs"] = logResults
